...
static void send_big_int(std::vector<uint32_t> const& arr, 
						 ...
						 int32_t bytes) {
	conn << bytes << '\n';
	int32_t count = (bytes + 3) >> 2;
	for (int32_t i = 0; i < count; i++)
		conn << arr[i] << ' ';
	conn << '\n';
}

static void receive_big_int(std::vector<uint32_t> &out,
							int32_t &bytes,
							std::iostream &conn) {
    conn >> bytes;
	int32_t count = (bytes + 3) >> 2;
...
{
	conn << static_cast<int32_t>(pk->bits) << ' ' << static_cast<int32_t>(pk->lbits) << '\n';
	send_mpz(pk->n, buf, conn);
	send_mpz(pk->u, buf, conn);
	send_mpz(pk->g, buf, conn);
	send_mpz(pk->h, buf, conn);
}

void receive_pk(dgk_pubkey_t *pk, std::vector<uint32_t> &buf, std::iostream &conn)
{
	int32_t bits, lbits;
	conn >> bits;
	conn >> lbits;
...
void dgk_encrypt_plain(mpz_t res, dgk_pubkey_t const*pub, mpz_t plaintext, gmp_randstate_t rnd) {
	mpz_t r;
	mpz_init(r);
